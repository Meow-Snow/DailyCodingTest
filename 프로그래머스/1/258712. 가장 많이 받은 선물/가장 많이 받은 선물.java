...
class Solution {
    public int solution(String[] friends, String[] gifts) {
        ...
        for (int i = 0; i < len; i++) idMap.put(friends[i], i);
        
        int[][] giftArr = new int[len][len];
        int[] giftIdxArr = new int[len];
        for (String gift : gifts) {
            String[] exchangers = gift.split(" ");
            giftArr[idMap.get(exchangers[0])][idMap.get(exchangers[1])]++;
        ...
        for (int i = 0; i < len; i++) {
            ...
            for (int j = 0; j < len; j++) {
                if (i == j) continue;                
                int giveCnt = giftArr[i][j];
                int takeCnt = giftArr[j][i];
